...
export const useEscalation = () => {
  ...
  const handleEscalation = async () => {
    ...
    try {
      ...
      const response = await NetworkService.post('/api/vin/escalation', escalationPayload);
      ...
      if (response.success) {
        ...
        Alert.alert('Error', 'Failed to register escalation. Please contact support.');
        return { success: false, error: response.error };
      }
    } catch (error) {
      console.error('Error creating escalation:', error);
      Alert.alert('Error', 'An error occurred while registering escalation. Please contact support.');
